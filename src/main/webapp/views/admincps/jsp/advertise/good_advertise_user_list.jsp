				...
				<div class="widget">
					...
					<div class="widget-body">
						<div role="grid" id="simpledatatable_wrapper"
							...
							<form id="searchForm"
								action="${basePath}backstage/advertise/index" method="post">
										...
										name="state" id="state" class="form-control input-sm">
											...
											<c:if test="${!empty stateMap}">
												<c:forEach items="${stateMap}" var="dp">
													<option value="${dp.key}"
														<c:if test="${dp.key eq state && !empty state}">selected="selected"</c:if>>${dp.value}</option>
												</c:forEach>
											</c:if>
									</select> <select name="adTag" id="adTag" class="form-control input-sm">
											<option value="">选择标签</option>
											<c:if test="${!empty adTagList}">
												<c:forEach items="${adTagList}" var="dp">
													<option value="${dp.codeValue}"
														<c:if test="${dp.codeValue eq adTag}">selected="selected"</c:if>>${dp.valueDesc}</option>
												</c:forEach>
											</c:if>
									</select> <select name="adType" id="adType"
										class="form-control input-sm">
											<option value="">选择类型</option>
											<c:if test="${!empty adTypeList}">
												<c:forEach items="${adTypeList}" var="dp">
													<option value="${dp.type}"
														<c:if test="${dp.type eq adType}">selected="selected"</c:if>>${dp.name}</option>
												</c:forEach>
											</c:if>
											...
											<c:forEach var="dp" varStatus="status" items="${pm.results}">
												<tr>
													<td><label> <input type="checkbox" name="ck"
															id="ck_${dp.adId}" value="${dp.adId}"> <span
															class="text"></span>
													</label></td>
													<c:if test="${!empty permitTBtn}">
													...
													<td class="text-center">${dp.adTypeName}</td>
													<td class="text-center"><c:if test="${dp.state==1}">
															<span class="label label-success">已上架</span>
														</c:if> <c:if test="${dp.state!=1}">
															<span class="label label-danger">已下架</span>
														</c:if></td>

													<td><c:if test="${!empty dp.adPicList}">
															<c:forEach var="pc" items="${dp.adPicList}"
																varStatus="st" end="0">
																<img class="lazy" src="${pc.avPic}"
													...
													<td>${dp.startTime}</td>
													<td>${dp.endTime}</td>
													<td>${dp.userInfo.loginAccount}</td>
