				...
				<div class="widget">
					...
					<div class="widget-body">
						<div role="grid" id="simpledatatable_wrapper"
							...
							<form action="${basePath}backstage/psAppInfo/index"
								method="post">
										...
										<c:if test="${!empty pm}">
											<c:forEach var="dp" varStatus="status" items="${pm.results}">
												<tr>
													...
													<td>${dp.appId}</td>
													<td>${dp.appName}</td>
													<td>${dp.packageName }</td>
													<td><c:if test="${fn:contains(dp.bzName,'1')}">微信好友</c:if>
													&nbsp;<c:if test="${fn:contains(dp.bzName,2)}">朋友圈</c:if></td>
												</tr>
											</c:forEach>
										</c:if>
