                                ...
                                <div class="widget-body">
									<div role="grid" id="simpledatatable_wrapper" class="dataTables_wrapper form-inline no-footer">
									   <c:if test="${!empty permitFBtn}">
										...
										<form action="${basePath}backstage//money/visit/index" method="post">
											...
											<c:if test="${!empty pm}">
											   <c:forEach var="dp" varStatus="status" items="${pm.results}">
												<tr>
													<td>
														<label>
				                                            <input type="checkbox" name="ck" id="ck_${dp.vmId}" value="${dp.vmId}">
				                                            <span class="text"></span>
				                                        </label>
													</td>
													<td>${dp.vmId}</td>
													...
													<td>${dp.memberInfo.memberName}</td>
													<td>${dp.memberInfo.payAccount}</td>
													<td>${dp.memberInfo.visitCode}</td>
													<td>${dp.invitedMemberInfo.loginAccount}</td>
													<td class="text-center"><fmt:formatNumber value="${dp.vmoney}" type="currency" pattern="0.00"/></td>
												    <td><fmt:formatDate value="${dp.createTime}" type="both" pattern="yyyy年MM月dd日 HH:mm:ss"/></td>
												</tr>
												   ...
												   <td colspan="15">
														...
														<div class="col-sm-6 no-padding">
															<div class="dataTables_paginate paging_bootstrap" id="simpledatatable_paginate">
																<ul class="pagination">
																	<pg:pager url="${basePath}backstage/money/visit/index"
