							...
							<div class="widget">
								<div class="widget-header bordered-bottom bordered-azure">
									...
									<div class="widget-buttons">
										<a href="#" data-toggle="maximize"> <i
											class="fa fa-expand" title="全屏展示"></i>
										</a> <a href="#" data-toggle="collapse"> <i
											class="fa fa-minus" title="展开缩小"></i>
										</a>
									</div>
								</div>
								<!--Widget Header-->
								<div class="widget-body">
									<div role="grid" id="simpledatatable_wrapper"
										...
										<form action="${basePath}backstage/pay/index" method="post">
											<div id="simpledatatable_filter" class="dataTables_filter1">
												<label class="no-margin-left"> <input type="text"
													...
													&nbsp;时间: <input class="input-sm Wdate"
													style="width: 100px;" type="text" name="start_date"
													id="start_date"
											...
											<table class="table table-striped table-bordered table-hover"
												...
												<thead>
													<tr role="row">
														...
														<th style="width: 50px; text-align: center">序 号</th>
												...
												<tbody>
													<c:if test="${!empty pm}">
														<c:forEach var="dp" varStatus="status"
															items="${pm.results}">
															<tr>
																<td><label> <input type="checkbox"
																		name="ck" id="ck_${dp.payId}" value="${dp.payId}">
																		<span class="text"></span>
																</label></td>
																<td style="text-align: center">${status.index+1}</td>
																<td style="width: 100px; text-align: center"><c:if
																		test="${dp.cashLog.payType==1}">
																		<span class="label label-success">${dp.cashLog.typeName}</span>
																	</c:if> <c:if test="${dp.cashLog.payType==0}">
																		<span class="label label-sky">${dp.cashLog.typeName}</span>
																	</c:if></td>
																<td style="width: 120px;">${dp.cashLog.payAccount}</td>
																<td style="width: 120px;">${dp.cashLog.accountName}</td>
																<%-- <td style="width: 120px;">${dp.payAccount}</td> --%>
																<td style="width: 120px;">${dp.payMoney}</td>
																<td style="width: 260px;">${dp.alipayCode}</td>
																<td style="width: 140px;"><fmt:formatDate
																		value="${dp.payTime}" type="both"
																		pattern="yyyy年MM月dd日 HH:mm:ss" /></td>
																<td>${dp.operater}</td>
																<td style="width: 140px;"><fmt:formatDate
																		value="${dp.createTime}" type="both"
																		pattern="yyyy年MM月dd日 HH:mm:ss" /></td>
																<td>${dp.remark}</td>
															...
															<td colspan="15">
																...
																<div class="col-sm-6 no-padding">
																	<div class="dataTables_paginate paging_bootstrap"
																		...
																		<ul class="pagination">
																			<pg:pager url="${basePath}backstage/pay/index"
																				items="${pm.totalRecord}"
																				export="current=pageNumber" maxIndexPages="5"
																				maxPageItems="${pm.pageSize}">
																				<pg:param name="totalRecord"
																					value="${pm.totalRecord}" />
																				...
																				<c:if
																					test="${current!=pm.totalPage && pm.totalPage>1}">
																					<pg:last>
																						<li class="prev"><a href="${pageUrl}"
																							class="next">尾页</a></li>
																					</pg:last>
