				...
				<div class="widget">
					...
					<div class="widget-body">
						<div role="grid" id="simpledatatable_wrapper"
							...
							<form action="${basePath}backstage/customtask/index"
								method="post">
											...
											<c:forEach var="dp" varStatus="status" items="${pm.results}">
												<tr>
													...
													<td style="width: 55px;" class="text-center"><c:if
														...
														</c:if> <c:if test="${dp.status==1}">
															<span class="label label-danger">停用</span>
														</c:if></td>
													<td style="width: 55px;" class="text-center">${dp.id}</td>
													<td>${dp.title}</td>
													<td><c:choose>
															<c:when test="${dp.type==1}">日常任务</c:when>
															<c:when test="${dp.type==2}">高佣任务</c:when>
															<c:when test="${dp.type==3}">新手任务</c:when>
															<c:when test="${dp.type==4}">积分墙任务</c:when>
															<c:when test="${dp.type==5}">限量任务</c:when>
															<c:when test="${dp.type==6}">其它任务</c:when>
													...
													<td><c:if test="${dp.target=='blank'}">新窗口打开</c:if>
														<c:if test="${dp.target=='self'}">本窗口打开</c:if></td>
												</tr>
											</c:forEach>
										</c:if>
												...
												<td colspan="9">
													...
													<div class="col-sm-6 no-padding">
														<div class="dataTables_paginate paging_bootstrap"
															...
															<ul class="pagination">
																<pg:pager url="${basePath}backstage/customtask/index"
