								...
								<div class="widget-body">
									<div role="grid" id="simpledatatable_wrapper" class="dataTables_wrapper form-inline no-footer">
									   ...
									   <c:if test="${!empty permitFBtn}">
										...
										<form id="searchForm" action="${basePath}backstage/sadvertise/index" method="post">
                                                ...
                                                <option value="">是否有效</option>
												  <c:if test="${!empty stateMap}">
												    <c:forEach items="${stateMap}" var="dp">
												       <option value="${dp.key}" <c:if test="${dp.key eq ad_state}">selected="selected"</c:if> >${dp.value}</option>
												    </c:forEach>
												  </c:if>
												</select>
											...
											<c:if test="${!empty pm}">
											   <c:forEach var="dp" varStatus="status" items="${pm.results}">
												<tr>
													...
													<td>
													<c:forEach var="po" varStatus="status" items="${adPosition}">
													<c:if test="${po.key==dp.adPosition}">
													${po.value}
													</c:if>
													</c:forEach>
													</td>
