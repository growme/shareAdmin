								...
								<div class="widget-body">
									<div role="grid" id="simpledatatable_wrapper" class="dataTables_wrapper form-inline no-footer">
									   ...
									   <c:if test="${!empty permitFBtn}">
										...
										<form id="searchForm" action="${basePath}backstage/member/index" method="post">
										<div id="simpledatatable_filter" class="dataTables_filter">
											<label class="no-margin-left">
												...
												<input type="text" name="memberId" id="memberId"placeholder="会员ID" class="form-control input-sm" value="${memberId}"/>
												<select name="member_state" id="member_state" class="form-control input-sm">
                                                <option value="">--所有类型--</option>
												  <c:if test="${!empty stlist}">
												    <c:forEach items="${stlist}" var="dp">
												       <option value="${dp.type}" <c:if test="${dp.type eq member_state}">selected</c:if> >${dp.name}</option>
												    </c:forEach>
												  </c:if>
												</select>
											...
											<c:if test="${!empty pm}">
											   <c:forEach var="dp" varStatus="status" items="${pm.results}">
												<tr>
													<td>
														<label>
				                                            <input type="checkbox" name="ck" id="ck_${dp.memberId}" value="${dp.memberId}">
				                                            <span class="text"></span>
				                                        </label>
													</td>
													<c:if test="${!empty permitTBtn}">
													...
													<c:forEach var="btn" items="${permitTBtn}">
													    <a href="javascript:${btn.btnFun}('${dp.memberId}');" id="${btn.btnId}" title="${btn.resourceName}"><i class="${btn.icon} sz14"></i></a>
													</c:forEach>
													</td>
													</c:if>
													...
													<td class="text-center">
													<span class="label label-${dp.showColor}">${dp.levelName}
													</td>
													<td>${dp.memberId}</td>
													<td>${dp.loginAccount}</td>
													<td>${dp.memberName}</td>
													<td>${dp.mobile}</td>
													<td>${dp.qqNum}</td>
													<td>${dp.wechat}</td>
													<td>${dp.payAccount}</td>
													<td>${dp.accountName}</td>
													<td>${dp.visitCode}</td>
													<td>${dp.recomUser}</td>
													<td>${dp.disscount}%</td>
													<td> 
												...
												<tr><td colspan="16" align="center" style="font-size:14px;color:red">很抱歉,没有查询到您要的数据</td></tr>
												</c:if>
												   ...
												   <td colspan="16">
														...
														<div class="col-sm-6 no-padding">
															<div class="dataTables_paginate paging_bootstrap" id="simpledatatable_paginate">
																<ul class="pagination">
																<pg:pager url="${basePath}backstage/member/index"
														         ...
														         <pg:param name="member_state" value="${member_state}" />
