...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body class="bodybg">
								...
								<div class="widget">
									...
									<div class="widget-body">
										<div role="grid" id="simpledatatable_wrapper" class="dataTables_wrapper form-inline no-footer">
											...
											<form action="${basePath}backstage/promote/index" method="post">
												...
												<c:if test="${!empty pm}">
												   <c:forEach var="dm" varStatus="status" items="${pm.results}">
													<tr>
														<td class="center">
															<label>
					                                            <input type="checkbox" name="ck" id="ck_${dm.linkId}" value="${dm.linkId}">
					                                            <span class="text"></span>
					                                        </label>
														</td>
														<c:if test="${!empty permitTBtn}">
														...
														<c:forEach var="btn" items="${permitTBtn}">
														    <a href="javascript:${btn.btnFun}('${dm.linkId}');" id="${btn.btnId}" title="${btn.resourceName}"><i class="${btn.icon} sz14"></i></a>
														</c:forEach>
														</td>
														</c:if>
														<td class="text-center" style="width:60px;">
														    <c:out value="${dm.linkId}"/>
														</td>
														<td class="text-center" style="width:80px;">
															<c:if test="${dm.state==0}">
															<span class="label label-success">${dm.stateName}</span>
															</c:if>
															<c:if test="${dm.state!=0}">
															<span class="label label-danger">${dm.stateName}</span>
															</c:if>
														</td>
														<td>
															<c:out value="${dm.linkAddr}"/>
														</td>
														<td class="text-center">
														    ${fn:substring(dm.updateTime, 0, 19)}
														</td>
														<td class="text-center">
															<c:out value="${dm.orderNo}"/>
														</td>
													</tr>
												   ...
												   <tr>
													...
													<div class="col-sm-6 no-padding">
														<div class="dataTables_paginate paging_bootstrap" id="simpledatatable_paginate">
															<ul class="pagination">
																<pg:pager url="${basePath}backstage/promote/index"
		...
		<script src="${basePath}static/js/admin/promote/promote.js"></script>
