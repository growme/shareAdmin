	                                ...
	                                <div class="widget-body">
										<div role="grid" id="simpledatatable_wrapper" class="dataTables_wrapper form-inline no-footer">
										   <c:if test="${!empty permitFBtn}">
											...
											<form action="${basePath}backstage/notice/index" method="post">
											...
											<table class="table table-striped table-bordered table-hover" id="simpledatatable" >
												<thead>
													<tr role="row">
														...
														<th style="width:65px;" class="text-center">置 顶</th>
												...
												<c:if test="${!empty pm}">
												   <c:forEach var="dp" varStatus="status" items="${pm.results}">
													<tr>
														<td>
															<label>
					                                            <input type="checkbox" name="ck" id="ck_${dp.noticeId}" value="${dp.noticeId}">
					                                            <span class="text"></span>
					                                        </label>
														</td>
														<c:if test="${!empty permitTBtn}">
														<td class="text-center" style="width:80px;">
														 <c:forEach var="btn" items="${permitTBtn}" varStatus="st">
														  <a href="javascript:${btn.btnFun}('${dp.noticeId}');" id="${btn.btnId}" title="${btn.resourceName}"><i class="${btn.icon} sz14"></i></a>
														 </c:forEach>
														</td>
														</c:if>
														...
														<td style="width:55px;" class="text-center">${dp.noticeId}</td>
														<td>${dp.noticeTitle}</td>
														<td>
														  ${dp.typeName}
														</td>
														
														<td>${dp.orderNumber}</td>
