									...
									<div class="widget-body">
										<div role="grid" id="simpledatatable_wrapper" class="dataTables_wrapper form-inline no-footer">
										  <c:if test="${!empty permitFBtn}" >
											...
											<form action="${basePath}backstage/taskschedule/index" method="post">
											...
											<table class="table table-striped table-bordered table-hover" id="simpledatatable" >
												<thead>
													<tr role="row">
														...
														<th>状态</th>
														<th>
															<i id="cronTh" title="cron表达式"></i>执行方式
														</th>
														<th>描述</th>
														<th>创建时间</th>
														<th>修改时间</th>
												...
												<c:if test="${!empty pm}">
												   <c:forEach var="prm" varStatus="status" items="${pm.results}">
													<tr>
														<td class="center">
															<label>
					                                            <input type="checkbox" name="ck" id="ck_${prm.taskScheduleId}" value="${prm.taskScheduleId}">
					                                            <span class="text"></span>
					                                        </label>
														</td>
														<c:if test="${!empty permitTBtn}">
														...
														<c:forEach var="btn" items="${permitTBtn}">
														    <a href="javascript:${btn.btnFun}('${prm.taskScheduleId}');" id="${btn.btnId}" title="${btn.resourceName}"><i class="${btn.icon}"></i></a>&nbsp;
														</c:forEach>
														</td>
														</c:if>
														<td>
															<c:out value="${prm.jobName}"/>
														</td>
														<td>
															<c:out value="${prm.aliasName}"/>
														</td>
														<td>
															<c:out value="${prm.jobGroup}"/>
														</td>
														<td>
															<c:out value="${prm.jobClass}"/>
														</td>
														<td>
															<c:if test="${prm.jobStatus==0}">
																<font color=green>启用</font>
														    </c:if>
														    <c:if test="${prm.jobStatus==1}">
														    	<font color=red>停用</font>
														    </c:if>
														</td>
														<td>
															<c:out value="${prm.cronExpression}"/>
														</td>
														<td style="width:200px;white-space:normal">
															<c:out value="${prm.description}"/>
														</td>
														<td>
															<fmt:formatDate value="${prm.createTime}" type="both" pattern="yyyy年MM月dd日 HH:mm:ss"/>  
														</td>
														<td>
															<fmt:formatDate value="${prm.updateTime}" type="both" pattern="yyyy年MM月dd日 HH:mm:ss"/>  
														</td>
													...
													<tr><td colspan="11" align="center" style="font-size:14px;color:red">很抱歉,没有查询到您要的数据</td></tr>
													</c:if>
													   ...
													   <td colspan="11">
														...
														<div class="col-sm-6 no-padding">
															<div class="dataTables_paginate paging_bootstrap" id="simpledatatable_paginate">
																<ul class="pagination">
																	<pg:pager url="${basePath}backstage/taskschedule/index"
