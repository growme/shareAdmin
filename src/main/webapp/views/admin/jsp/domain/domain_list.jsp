								...
								<div class="widget">
									...
									<div class="widget-body">
										<div role="grid" id="simpledatatable_wrapper" class="dataTables_wrapper form-inline no-footer">
											...
											<form action="${basePath}backstage/domain/index" method="post" id="searchForm">
											<div id="simpledatatable_filter" class="dataTables_filter">
												<label class="no-margin-left">
													...
													<select name="domainType" id="domainType" class="form-control input-sm">
	                                                <option value="">--所有类型--</option>
													  <c:if test="${!empty tlist}">
													    <c:forEach items="${tlist}" var="dp">
													       <option value="${dp.type}" <c:if test="${dp.type eq domainType}">selected</c:if> >${dp.name}</option>
													    </c:forEach>
													  </c:if>
													</select>
													&nbsp;
	                                                ...
	                                                <option value="">--所有状态--</option>
													  <c:if test="${!empty slist}">
													    <c:forEach items="${slist}" var="dp">
													       <option value="${dp.state}" <c:if test="${dp.state eq enabled}">selected</c:if> >${dp.name}</option>
													    </c:forEach>
													  </c:if>
													</select>
													&nbsp;
												...
												<c:if test="${!empty pm}">
												   <c:forEach var="dm" varStatus="status" items="${pm.results}">
													<tr>
														<td class="center">
															<label>
					                                            <input type="checkbox" name="ck" id="ck_${dm.domainId}" value="${dm.domainId}">
					                                            <span class="text"></span>
					                                        </label>
														</td>
														<c:if test="${!empty permitTBtn}">
														...
														<c:forEach var="btn" items="${permitTBtn}">
														    <a href="javascript:${btn.btnFun}('${dm.domainId}');" id="${btn.btnId}" title="${btn.resourceName}"><i class="${btn.icon} sz14"></i></a>
														</c:forEach>
														</td>
														</c:if>
														<td class="text-center" style="width:60px;">
														    <c:out value="${dm.domainId}"/>
														</td>
														<td class="text-center">
															<c:if test="${dm.enabled==0}">
															<span class="label label-success">${dm.enabledName}</span>
															</c:if>
															<c:if test="${dm.enabled!=0}">
															<span class="label label-danger">${dm.enabledName}</span>
															</c:if>
														</td>
														<td>
															<c:out value="${dm.domainName}"/>
														</td>
														<td class="text-center">
															<c:out value="${dm.domainTypeName}"/>
														</td>
														<td class="text-center">
															<c:out value="${dm.orderNumber}"/>
														</td>
														<td class="text-center">
														    ${fn:substring(dm.createTime, 0, 19)}
														</td>
														<td class="text-center">
														    ${fn:substring(dm.lastUpdateTime, 0, 19)}
														</td>
													</tr>
												   ...
												   <tr>
													...
													<div class="col-sm-6 no-padding">
														<div class="dataTables_paginate paging_bootstrap" id="simpledatatable_paginate">
															<ul class="pagination">
																<pg:pager url="${basePath}backstage/domain/index"
														           ...
														           <pg:param name="domainType" value="${domainType}" />
