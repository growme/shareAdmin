...
Date.MonthNames = new Array('一月', '二月', '三月', '四月', '五月', '六月', '七月', '八月',
		'九月', '十月', '十一月', '十二月');
...
Date.prototype.format = function(format) {
	...
	var weekInYear = Math
			.ceil((daysInYear + new Date(year, 0, 1).getDay()) / 7);
	var weekInMonth = Math.ceil((date + new Date(year, month, 1).getDay()) / 7);
	return format.replace("yyyy", year)
			.replace("yy", year.toString().substr(2)).replace("dd",
					(date < 10 ? "0" : "") + date).replace("HH",
					(hr < 10 ? "0" : "") + hr).replace("KK",
					(hr % 12 < 10 ? "0" : "") + hr % 12).replace("kk",
					(hr > 0 && hr < 10 ? "0" : "") + (((hr + 23) % 24) + 1))
			.replace(
					"hh",
					(hr > 0 && hr < 10 || hr > 12 && hr < 22 ? "0" : "")
							+ (((hr + 11) % 12) + 1)).replace("mm",
					(min < 10 ? "0" : "") + min).replace("ss",
					(sec < 10 ? "0" : "") + sec).replace("SSS", this % 1000)
			.replace("a", (hr < 12 ? "AM" : "PM")).replace("W", weekInMonth)
			.replace("F", Math.ceil(date / 7))
			.replace(/E/g, Date.DayNames[day]).replace("D", daysInYear)
			.replace("w", weekInYear).replace(/MMMM+/, sMonth).replace("MMM",
					sMonth.substring(0, 3)).replace("MM",
					(month < 9 ? "0" : "") + (month + 1));
}
...
function getServerTime() {
	$.ajax({
		type : 'POST',
		url : ccnetpath + '/backstage/getServerTime',
		data : {
			tm : new Date().getTime()
		},
		dataType : 'json',
		success : function(data, textStatus) {
			var ndate;
			var fmtStr = "yyyy年MM月dd日 HH:mm:ss&nbsp;&nbsp;E";
			if ("1" == data.res) {
				ndate = data.resMsg.split(",");
			} else {
				ndate = getNowFormatDate().split(",");
			}
			var srvClock = new ServerClock(ndate[0], ndate[1], ndate[2],ndate[3], ndate[4], ndate[5]);
			srvClock.set_delay(3000);
			window.setInterval(function() {
				$("#nowtime").html(srvClock.get_ServerTime(fmtStr));
			}, 500);
		}
	});
}
