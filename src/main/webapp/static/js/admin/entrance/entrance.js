...
$(function () {
	...
	$("#th_add_btn").click(function () {
		openPage("增加入口",500,320,ccnetpath+"/backstage/entrance/goadd");
	});
	...
	$("#th_edit_btn").click(function () {
        ...
        openPage("修改入口",400,320,ccnetpath + "/backstage/entrance/goedit?link_id="+ids);
	});
...
function saveEntrance(){
    ...
    var reqUrl = "/backstage/entrance/save";
    if(!isEmpty(linkId)){
    	reqUrl = "/backstage/entrance/edit";
    }
    ...
    if(isEmpty(id)){
	  ...
	  showTConFirm('您确定的要删除吗？',function(){
	        ...
	        $.ajax({
				   type: "POST",
				   cache : false,
				   url: ccnetpath+"/backstage/entrance/trash",
				   data: param,
				   dataType:'json',
...
function editEntrance(id){
   ...
   openPage("修改域名",400,320,ccnetpath + "/backstage/entrance/goedit?link_id="+ids);
}
