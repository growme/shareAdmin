...
$(function() {
	...
	$("#th_edit_btn").click(
			function() {
				...
				openPage("编辑自定义任务", 600, 510, ccnetpath
						+ "/backstage/psAppInfo/goedit?psAppInfo_id=" + ids);
			});
	...
	$("#saveBtn").click(function() {
		...
		if (isEmpty($("#appName").val())) {
			showErrMsg("对不起，app名称不能为空!");
			return false;
		}

		if (isEmpty($("#packageName").val())) {
			showErrMsg("对不起，包名不能为空!");
			return false;
		}
		
		var reqUrl = "/backstage/psAppInfo/save";
		if (!isEmpty(psAppInfoId)) {
			reqUrl = "/backstage/psAppInfo/edit";
		}
		...
		// 提交表单
		var datas = $("#psAppInfoForm").serialize();
...
function delpsAppInfo(id) {
	...
	showConFirm('您确定的要删除吗？', function() {
		...
		$.ajax({
			type : "POST",
			cache : false,
			url : ccnetpath + "/backstage/psAppInfo/trash",
			data : param,
			dataType : 'json',
			success : function(data, textStatus) {
				if ("1" != data.res) {
