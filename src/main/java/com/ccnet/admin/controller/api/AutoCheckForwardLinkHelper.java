 ...
 *
 */
public class AutoCheckForwardLinkHelper {
     ...
     * 检测入口任务
     */
    public static void autoCheckForwardLink() {
    	//获取域名检测的参数
    	...
    	List<SbForwardLink> linksList = forwardLinkService.getCheckValidForwardLinkList();
    	if(CPSUtil.isNotEmpty(api_user_id) && CPSUtil.isNotEmpty(api_user_key)){
    	     if(CPSUtil.listNotEmpty(linksList)){
	    		for (SbForwardLink link : linksList) {
	    			...
	    			if(CPSUtil.isNotEmpty(linkState)){
	    				try {
	    					...
	    					if(linkState.equals(DomainStateType.InValid.getState()+"")){
		        				...
		        				forwardLinkService.editSbForwardLinkInfo(link);
		        				CPSUtil.xprint("全局入口检测频率间隔："+sleep_time);
		        				
		        				//发送短信预警
		        				sendNoticeSms("跳转入口",link.getLinkAddr());
		        				//域名检测被封发送邮件
		        				sendNoticeMail("跳转入口",link.getLinkAddr());
		        				
		        				if(CPSUtil.isEmpty(sleep_time)){
		        					Thread.sleep(2000);
		        					CPSUtil.xprint("域名检测频率间隔：2秒");
		        				}else{
